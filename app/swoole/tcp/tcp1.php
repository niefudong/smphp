...
run(function () {
    ...
    $sendMessage = packData($sendMessage,$packModel);
    ...
    Swoole\Timer::tick(3*1000, function (int $timer_id, $client) use ($packModel) {
        $heartCheckData = [];
        $swoole_mysql = swooleMysql();

        $res = $swoole_mysql->query("select `token` from `dev_device` where `sn`='002' limit 1;");
        if($res){
